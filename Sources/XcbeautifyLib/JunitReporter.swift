...
private final class JunitComponentParser {
    ...
    func parse(component: JunitComponent) {
        ...
        case let .testSuiteStart(line):
            ...
            let groups = NewRegexMatcher(pattern: .testSuiteStart).capturedGroups(string: line)
            mainTestSuiteName = groups[0]

        case let .failingTest(line):
            let groups = NewRegexMatcher(pattern: .failingTest).capturedGroups(string: line)
        ...
        case let .testCasePassed(line):
            let groups = NewRegexMatcher(pattern: .testCasePassed).capturedGroups(string: line)
